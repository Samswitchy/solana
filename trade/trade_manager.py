...
def check_take_profit_and_sell(token_mint, bought_price, amount, tokens_sold):
    ...
    for level in TAKE_PROFIT_LEVELS:
        if profit_multiplier >= level and tokens_sold < amount:
            sell_amount = (amount - tokens_sold) * 0.50  # Sell 50% of remaining holdings
            ...
            if txid:
                ...
                new_tokens_sold = tokens_sold + sell_amount
                update_token_data(token_mint, "tokens_sold", new_tokens_sold)
                update_token_data(token_mint, "take_profit_level", level)
                
                if new_tokens_sold >= amount:
                    update_token_data(token_mint, "status", "SOLD")
                
                return txid
    return None

def check_stop_loss_and_sell(token_mint, bought_price, highest_price, amount, tokens_sold):
    ...
    if current_price < stop_loss_price:
        print(f"🚨 Stop-loss triggered for {token_mint}! Selling all remaining tokens.")
        remaining_tokens = amount - tokens_sold
        txid, received = sell_token(token_mint, OUTPUT_MINT, remaining_tokens, PRIORITY_FEE)
        if txid:
            print(f"✅ Sold {remaining_tokens} of {token_mint} due to stop-loss - Tx: {txid}")
...
def monitor_market():
    ...
    while True:
        ...
        if not tokens:
            ...
            for token in tokens:
                token_mint, amount, _, bought_price, highest_price, tokens_sold, status = token
                
                if status == "SOLD":
                    continue  # Skip already sold tokens
                
                # Check take-profit first
                txid = check_take_profit_and_sell(token_mint, bought_price, amount, tokens_sold)

                # If not sold at profit, check stop-loss
                if not txid:
                    check_stop_loss_and_sell(token_mint, bought_price, highest_price, amount, tokens_sold)

        print("⏳ Waiting before next check...")
        time.sleep(2)  # Adjust based on preference
