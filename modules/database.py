...
async def move_to_graduating_db(token_address, marketCap):
    ...
    pot_token = 55000 if marketCap >= 55000 else "NO"
     ...
     # ✅ Fetch volume trend for the token
    volume_data = await get_volume_trend(token_address)

    # Extract trend from the result
    volume_trend = volume_data.get(token_address, {}).get("trend", "Stable")

    # ✅ Determine trade decision
    trade = "BUY" if volume_trend == "Uptrend ✅" and marketCap < 55000 else "HOLD"



    async with aiosqlite.connect(GRADUATING) as conn:
        await conn.execute("""
            INSERT INTO graduating_tokens (token_address, marketCap, pot_token, trade) 
            VALUES (?, ?, ?, ?)
            ON CONFLICT(token_address) DO UPDATE SET 
            marketCap = excluded.marketCap,
            pot_token = excluded.pot_token;
            trade = excluded.trade;
        """, (token_address, marketCap, pot_token,trade))
        await conn.commit()
...
async def fetch_graduating_tokens():
    ...
    # ✅ Ensure 'ath' is never None by setting a default value of 0
    return [(row[0], row[1], row[2] if row[2] is not None else 0) for row in tokens]
